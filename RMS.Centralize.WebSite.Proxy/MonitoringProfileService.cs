...
using RMS.Centralize.WebSite.Proxy.MonitoringProfileProxy;
...
namespace RMS.Centralize.WebSite.Proxy
{
    public class MonitoringProfileService
    {
        ...
        MonitoringProfileServiceClient _MonitoringProfileService;

        #region Constructor

        public MonitoringProfileService()
            : this(null)
        {
        }

        public MonitoringProfileService(string urlWebService)
        {

            try
            {
                _MonitoringProfileService = new MonitoringProfileServiceClient();
                Initialize(urlWebService);
            }
            catch (Exception ex)
            {
                _MonitoringProfileService = null;
                throw new RMSWebException(this, "0500", "MonitoringProfileService ctor failed. " + ex.Message, ex, false);
            }
        }
        ...
        public MonitoringProfileServiceClient monitoringProfileService
        {
            get { return _MonitoringProfileService; }
            set { _MonitoringProfileService = value; }
        }
            ...
            try
            {
                try
                {
                    if (urlWebService != null)
                        ...
                        webServicURL = ConfigurationManager.AppSettings["RMS.WebServicURL_MonitoringProfileService"];
                ...
                _MonitoringProfileService.Endpoint.Address = new EndpointAddress(webServicURL);
                if (timeOut != null)
                    _MonitoringProfileService.Endpoint.Binding.SendTimeout = new TimeSpan(0, 0, timeOut.Value);

                var b = _MonitoringProfileService.Endpoint.Binding as System.ServiceModel.BasicHttpBinding;
