...
namespace RMS.Centralize.WebSite.Proxy
{
    public class ClientMonitoringService
    {
        ...
        ClientMonitoringServiceClient _ClientMonitoringService;

        #region Constructor

        public ClientMonitoringService()
            : this(null)
        {
        }

        public ClientMonitoringService(string urlWebService)
        {

            try
            {
                _ClientMonitoringService = new ClientMonitoringServiceClient();
                Initialize(urlWebService);
            }
            catch (Exception ex)
            {
                _ClientMonitoringService = null;
                throw new RMSWebException(this, "0500", "ClientMonitoringService ctor failed. " + ex.Message, ex, false);
            }
        }
        ...
        public ClientMonitoringServiceClient clientMonitoringService
        {
            get { return _ClientMonitoringService; }
            set { _ClientMonitoringService = value; }
        }
            ...
            try
            {
                try
                {
                    if (urlWebService != null)
                        ...
                        webServicURL = ConfigurationManager.AppSettings["RMS.WebServicURL_ClientMonitoringService"];
                ...
                _ClientMonitoringService.Endpoint.Address = new EndpointAddress(webServicURL);
                if (timeOut != null)
                    _ClientMonitoringService.Endpoint.Binding.SendTimeout = new TimeSpan(0, 0, timeOut.Value);

                var b = _ClientMonitoringService.Endpoint.Binding as System.ServiceModel.BasicHttpBinding;
