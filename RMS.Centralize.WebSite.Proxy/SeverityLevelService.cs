...
{
    public class SeverityLevelService
    {
        ...
        SeverityLevelServiceClient _SeverityLevelService;
        ...
        public SeverityLevelService(string urlWebService)
        {

            try
            {
                _SeverityLevelService = new SeverityLevelServiceClient();
                Initialize(urlWebService);
            }
            catch (Exception ex)
            {
                _SeverityLevelService = null;
        ...
        public SeverityLevelServiceClient severityLevelService
        {
            get { return _SeverityLevelService; }
            set { _SeverityLevelService = value; }
        }
            ...
            try
            {
                try
                {
                    if (urlWebService != null)
                        ...
                        webServicURL = ConfigurationManager.AppSettings["RMS.WebServicURL_SeverityLevelService"];
                ...
                _SeverityLevelService.Endpoint.Address = new EndpointAddress(webServicURL);
                if (timeOut != null)
                    _SeverityLevelService.Endpoint.Binding.SendTimeout = new TimeSpan(0, 0, timeOut.Value);

                var b = _SeverityLevelService.Endpoint.Binding as System.ServiceModel.BasicHttpBinding;
