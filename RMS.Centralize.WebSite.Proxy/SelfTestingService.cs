...
using RMS.Centralize.WebSite.Proxy.SelfTestingProxy;
...
namespace RMS.Centralize.WebSite.Proxy
{
    public class SelfTestingService
    {
        ...
        SelfTestingServiceClient _selfTestingService;

        #region Constructor

        public SelfTestingService() : this(null)
        {
        }

        public SelfTestingService(string urlWebService)
        {
            ...
            {
                _selfTestingService = null;
                throw new RMSWebException(this, "0500", "SelfTestingService ctor failed. " + ex.Message, ex, false);
        ...
        public SelfTestingServiceClient selfTestingService
        {
            get { return _selfTestingService; }
            set { _selfTestingService = value; }
        }
            ...
            try
            {
                try
                {
                    if (urlWebService != null)
                        ...
                        webServicURL = ConfigurationManager.AppSettings["RMS.WebServicURL_SelfTestingService"];
                ...
                _selfTestingService.Endpoint.Address = new EndpointAddress(webServicURL);
                if (timeOut != null)
                    _selfTestingService.Endpoint.Binding.SendTimeout = new TimeSpan(0, 0, timeOut.Value);

                var b = _selfTestingService.Endpoint.Binding as System.ServiceModel.BasicHttpBinding;
