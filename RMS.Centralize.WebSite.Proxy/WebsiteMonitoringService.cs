...
using RMS.Centralize.WebSite.Proxy.WebsiteMonitoringProxy;
...
namespace RMS.Centralize.WebSite.Proxy
{
    public class WebsiteMonitoringService
    {
        ...
        WebsiteMonitoringServiceClient _WebsiteMonitoringService;
        

        #region Constructor

        public WebsiteMonitoringService() : this(null)
        {
        }

        public WebsiteMonitoringService(string urlWebService)
        {

            try
            {
                _WebsiteMonitoringService = new WebsiteMonitoringServiceClient();
                Initialize(urlWebService);
            }
            catch (Exception ex)
            {
                _WebsiteMonitoringService = null;
                throw new RMSWebException(this, "0500", "WebsiteMonitoringService ctor failed. " + ex.Message, ex, false);
        ...
        public WebsiteMonitoringServiceClient websiteMonitoringService
        {
            get { return _WebsiteMonitoringService; }
            set { _WebsiteMonitoringService = value; }
        }
            ...
            try
            {
                try
                {
                    if (urlWebService != null)
                        ...
                        webServicURL = ConfigurationManager.AppSettings["RMS.WebServicURL_WebsiteMonitoringService"];
                ...
                _WebsiteMonitoringService.Endpoint.Address = new EndpointAddress(webServicURL);
                if (timeOut != null)
                    _WebsiteMonitoringService.Endpoint.Binding.SendTimeout = new TimeSpan(0, 0, timeOut.Value);

                var b = _WebsiteMonitoringService.Endpoint.Binding as System.ServiceModel.BasicHttpBinding;
