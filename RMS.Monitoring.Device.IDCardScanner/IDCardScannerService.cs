...
{
    public class IDCardScannerService
    {
        private IDCardScanner _device;
        private ClientResult clientResult;

        public IDCardScannerService(string brand, string model, string deviceManagerName, string deviceManagerID, ClientResult clientResult)
        {
            try
            {
                this.clientResult = clientResult;

                if (brand.ToLower() == "syscantech") _device = new Syscantech(model, deviceManagerName, deviceManagerID);
                else
                    throw new Exception("Brand Not Found. brand=" + brand);
            }
            catch (Exception ex)
            {
                throw new RMSAppException(this, "0500", "IDCardScannerService failed. " + ex.Message, ex, false);
            }
        }
